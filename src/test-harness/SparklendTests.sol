...
import { StdChains, console } from "forge-std/Test.sol";
...
abstract contract SparklendTests is SpellRunner {
    ...
    function _e2eTestFlashLoan(
        ...
        pool.flashLoanSimple(
            address(this), // TODO: MDL, make this a contract.
    ...
    function _writeReserveConfigs(
        ...
        for (uint256 i = 0; i < configs.length; ++i) {
            ...
            vm.serializeAddress(
                key,
                "aTokenImpl",
                ProxyHelpers.getInitializableAdminUpgradeabilityProxyImplementation(config.aToken)
            );
            ...
            vm.serializeAddress(
                key,
                "stableDebtTokenImpl",
                ProxyHelpers.getInitializableAdminUpgradeabilityProxyImplementation(config.stableDebtToken)
            );
            vm.serializeString(
                key,
            ...
            vm.serializeAddress(
                ...
                ProxyHelpers.getInitializableAdminUpgradeabilityProxyImplementation(config.variableDebtToken)
            );
            vm.serializeString(
                key,
    ...
    function _writePoolConfiguration(string memory path, IPool pool) internal {
        ...
        vm.serializeAddress(
            poolConfigKey,
            "poolImpl",
            ProxyHelpers.getInitializableAdminUpgradeabilityProxyImplementation(address(pool))
        );
    ...
    function _requireNoReservesConfigsChangesApartFrom(
        ...
        for (uint256 i = 0; i < allConfigsBefore.length; ++i) {
            ...
            if (isAssetExpectedToChange) continue;
    ...
    function _validateReserveTokensImplementations(
        ...
        require(
            IProxyLike(config.variableDebtToken).implementation() ==
                expectedImplementations.variableDebtToken,
            "_validateReserveTokensImplementations() : INVALID_ATOKEN_IMPL"
        );
