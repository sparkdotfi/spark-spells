...
- **Orphaned capabilities (unreachable roles)** — a role held by a contract that itself has no
  operable caller is a dead capability: the permission exists but no human can ever exercise it.
  Walk the full authority chain from a human-operable origin (multisig signers / documented EOA)
  to the final state-changing function, verifying every hop on-chain (live RPC), and enumerate
  roles from deployed ground truth (`RoleGranted` logs / `hasRole` probes) — never from the
  repo-vendored contract source, whose role names may differ from the deployed version.
- **Reference-deployment parity** — when activating pre-deployed infra on a new chain, diff the
  full role/config state of EVERY contract in the deployment (including auxiliary ones the spell
  never touches) against the latest working deployment on a reference chain; any role present on
  the reference but absent on the new chain must be spec-justified or flagged.
...
- **`vm.prank` of a privileged intermediate contract** — pranking a contract address assumes,
  without proof, that someone can operate that contract in production. Each new operational path
  needs at least one test starting from the true production origin (the actual multisig/relayer)
  traversing every real contract hop; if that test cannot be written, that is itself a finding.
