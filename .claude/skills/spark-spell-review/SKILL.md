...
(footguns §14). Two mandatory deep checks:

- **Privilege-reachability (authority-chain) audit.** For EVERY privileged role the spell grants,
  revokes, or relies on — including roles set at deployment time on infrastructure the spell
  activates — construct the full authority chain from a human-operable origin (multisig signers /
  documented EOA) to the final state-changing function (e.g. relayer multisig → ALLOCATOR_ROLE on
  ALMProxyFreezable → doCall → SETTER_ROLE on vault → setVsr). Verify each hop ON-CHAIN via live
  RPC, never from repo source or registry constants: (a) the address has deployed code on THAT
  chain (or is an intended EOA); (b) if it is a contract, confirm at least one address holds its
  operator role(s) — a role held by a contract that itself has no operators is an ORPHANED
  CAPABILITY, 🔴 Critical; (c) if it is a multisig, read owners/threshold and compare to spec.
  Enumerate roles from deployed-contract ground truth (RoleGranted logs / hasRole probes), NOT the
  repo-vendored contract version — deployed role names may differ. For each new product, walk the
  day-2 ops lifecycle (set rate, take, replenish, freeze/unfreeze) and confirm each action has a
  currently-operable caller on that chain.
- **Reference-deployment parity diff.** When activating pre-deployed infra on a new chain,
  enumerate EVERY contract in the deployment (including auxiliary ones the spell never touches)
  and diff full role/config state against the latest working deployment on a reference chain. Any
  role present on the reference but absent on the new chain must be spec-justified or flagged.

Report every check that failed to run (no RPC, unverified source) as unverified.
...
`chainData` payload addresses vs constructor `PAYLOAD_*`; determinism (relative warps). Apply the
**`vm.prank` red-flag rule**: every `vm.prank(<contract address>)` of a privileged intermediate
contract is a coverage smell — it assumes, without proof, that someone can operate that contract
in production. Require at least one test per new operational path that starts from the true
production origin (the actual multisig/relayer) and traverses every real contract hop; if that
test cannot be written, that is itself a finding. Independently enumerate the deployment's
contracts and confirm each has config-state test coverage — do not inherit the existing test
suite's blind spots. Then run the suite if the environment allows:
`forge test --match-path 'src/proposals/<date>/*'` (needs RPC env vars; try, and report inability
honestly). Apply footguns §10–11.
